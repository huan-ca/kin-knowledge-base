...
- source-example-scripted-lesson-plan-adult#chunk-002
- source-example-scripted-lesson-plan-youth#chunk-001
- source-example-scripted-lesson-plan-youth#chunk-003
related_pages: []
---
...
- The example begins with week metadata before any script content.
- The example includes an opening script rather than only technical bullet points.
- The example separates warm-up options from the main lesson block.
- The example breaks the lesson into named technical sub-blocks and ends with situational options.

## Preserved Example Format

````markdown
 Example lesson plan format for adults:
...
- **Movement Based**:
  - Duck walks
  - Break falls
  - Bear crawls with backsteps
  ...
- **Drill Based**:
  - Double leg pickups
  - Double leg penitrations
  ...
- **CLA Based**:
  ....
...
- **Takedown**
  ...
  - Level 1: Double leg (low amplitude/high amplitude)
    ...
    - lock behind knees, lift with both feet
  - Level 2: Double leg (chain attack)
    - Example: High crotch, single leg, duck under/sit out
...
- The example does not define when this lesson should be used in a larger approved calendar.
