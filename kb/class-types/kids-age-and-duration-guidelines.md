---
id: kids-age-and-duration-guidelines
type: class-type
title: "Kids Age and Duration Guidelines"
status: active
confidence: 0.9
claim_label: fact
source_refs:
- source-kin-bjj-compendium#chunk-007
- source-kin-bjj-kids-lesson-plan-structure#chunk-001
- source-kin-bjj-kids-lesson-plan-structure#chunk-002
related_pages: []
---
# Kids Age and Duration Guidelines

## Definition
The gym separates kids programming into tots and youth cohorts with recommended durations matched to attention span and maturity.

## Detailed Notes
- Tots are generally ages 4 to 7.
- Youth are generally ages 8 to 13.
- Tots classes are typically 30 to 45 minutes depending on source phrasing and age.
- Youth classes are typically 60 minutes.
- Students can be placed by suitability rather than age alone.

## Gaps
- The exact threshold for moving between programs is coach-driven rather than fully codified.
